...
let user =
    freya {
        ...
        match email with
        | Some e ->
            return! Auth.userExists e |> Freya.fromJob
        | None ->
            return None
    }
...
module WritingStuffApi =
    let stuffId =
        Freya.Optic.get <| Route.atom_ "stuffId"
    ...
    let eventsPosted getState store freyaCmds =
        freya {
            ...
            | Some i ->
                let! state =
                    getState
                    |> Freya.map (function Some s -> s | None -> WritingStuff.Empty)
                do!
                    cmds
